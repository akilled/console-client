    ...
    class Protocol18Handler : IMinecraftCom
    {
        ...
        private bool handlePacket(int packetID, List<byte> packetData)
        {
            ...
            if (packetID == 0x2D)
            {
                ...
                {
                    window_title = window_title.Substring(29).Trim();
                }
            ...
            if (packetID == 0x30 && chest_open)
            {
                ...
                int given_id = captcha[chest_title];
                for (int i = 0; i < chest_size; i++)
                {
                    ushort item_id = readNextUShort(packetData);
        ...
        public Dictionary<string, int> captcha = new Dictionary<string, int>
        {
            { "AIR", 0 },
            { "STONE", 1 },
            { "GRASS", 2 },
